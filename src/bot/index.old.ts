...
import { CollectionWithMedia, CollectionListItem, CollectionFilters } from '../types/collection';
...
// 工具函数：显示编辑合集界面
async function showEditCollectionUI(ctx: MyContext, collection: CollectionWithMedia, collectionId: number) {
...
// 辅助函数：构建合集列表消息和键盘
function buildCollectionListMessage(collections: CollectionListItem[], total: number, page: number, totalPages: number, keyword?: string, isAdmin: boolean = false) {
